...
notion-mcp-serverの設計思想と主要なアーキテクチャについて解説する。
...
* ページ取得:
    ページ内容（JSON形式）を常にローカルファイル（例: `/workspace/notion-mcp-server-page-...`）に保存し、LLMには「ファイルパス」と「メタデータ」のみを返却する。これにより、消費トークンを数百程度に抑えられる。
* デフォルト Extract:
